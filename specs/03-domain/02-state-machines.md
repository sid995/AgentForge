...
stateDiagram-v2
    ...
    SCHEDULING --> POLICY_REJECTED: scheduling admission rejection
...
| `SCHEDULING` | `POLICY_REJECTED` | Scheduler admission policy permanently rejects the run | Claim is still owned; the current eligibility decision is a permanent rejection | Clear lease; persist bounded decision code/reason and policy failure category; cancel and complete the pending attempt; complete the run | Exact replay with the same decision returns the prior result; a changed decision conflicts | `agent_run.policy_rejected` | `agent-run.failed.v1` |
